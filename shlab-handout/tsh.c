...
volatile sig_atomic_t pid;  /* flag bit used for waiting for foreground job */
...
void eval(char *cmdline) 
{
    ...
    //           printf("main process pid: %d\n", getpid());
            ...
            if((fork_pid = fork()) == 0){
                sigemptyset(&mask_prev);
                sigprocmask(SIG_SETMASK, &mask_prev, NULL); //unblock all blocking signal 
                setpgid(0, 0); /*use the pid of child process to create a new group*/
                if(execve(argv[0], argv, environ) < 0){
                   printf("%s: Command not found.\n", argv[0]);
                   exit(0);
                }
                ...
                struct job_t* bg_job = getjobpid(jobs,fork_pid);
                printf("[%d] (%d) %s", bg_job->jid, bg_job->pid, bg_job->cmdline);
                sigprocmask(SIG_SETMASK, &mask_prev, NULL); //unblock all blocking signal 
            }
        }
    }
...
int builtin_cmd(char **argv) 
{
    if(!strcmp(argv[0], "quit")) /* quit command */
        exit(0);
    if(!strcmp(argv[0], "&")) /*ignore sigleton & */
        return 1;
...
void do_bgfg(char **argv) 
{
    if(!strcmp(argv[0], "bg")){
        ...
        if(job){
            if(job->state == BG){
                printf("Job [%d] already in backgroun.\n", job->jid);
                return;
            }
            job->state = BG;
...
void sigchld_handler(int sig) 
{
   //printf("SIGCHLD handler working\n");
    sigset_t mask_all, mask_prev;
    sigemptyset(&mask_all);
    sigfillset(&mask_all);
    pid_t tempt_pid;
    int olderrno = errno;
    while((tempt_pid = waitpid(-1, NULL, WNOHANG | WUNTRACED)) > 0){
    //    printf("SIGCHLD working\n");
        sigprocmask(SIG_BLOCK, &mask_all, &mask_prev);
        struct job_t * job = getjobpid(jobs, tempt_pid);
        if(job){
            if(job->state == FG || job->state == ST)
                pid = tempt_pid;
            if(job->state == FG || job->state == BG){
                deletejob(jobs, tempt_pid);
            }
        }
        sigprocmask(SIG_SETMASK, &mask_prev, NULL);
    }
...
void sigint_handler(int sig) 
{
     //printf("SIGINT is working | process %d\n", getpid());
   // kill(getpid(),SIGCHLD);
    sigset_t mask_all, mask_prev;
    sigemptyset(&mask_all);
    sigfillset(&mask_all);
    ...
    sigprocmask(SIG_BLOCK, &mask_all, &mask_prev);
    int fore_jid = maxjid(jobs);
    struct job_t * job = getjobjid(jobs, fore_jid);
    if(job){
        if(job->state == FG)
        {
