...
int isAsciiDigit(int x) {
  /* negative(x) plus lower bound; negative(x) plus upper bound; check the sign bit */
  int negX = ~x + 1;
  int lowerBound = ~0x30 + 1 + x;
  int upperBound = negX + 0x39;
...
int howManyBits(int x) {
  ...
  int sign = x >> 31;
  x = (sign & ~x) | (~sign & x); //sign bit map 0xffffffff / 0x00000000  -- x为负数则取～

  b16 = !!(x >> 16) << 4; //x的right most significant 16 bits是否包含1，包含则b16记录数值，并将x右移16bits
  x = x >> b16;
  b8 = !!(x >> 8) << 3; //经过右移b16(0或者16) bits的 x的 right most significant 8 bits是否包含1，包含则记录b8的值，x右移b8(0或8)位
  x = x >> b8;
  b4 = !!(x >> 4) << 2;
  x = x >> b4;
  b2 = !!(x >> 2) << 1;
